      ...
      <el-card body-style="padding: 30px;">
        ...
        <el-descriptions title="" border :column="3" style="margin-bottom: 24px;">
          ...
          <el-descriptions-item label="发文单位">
            <el-input v-model="doc.senderOrg" placeholder="请输入发文单位" size="mini"></el-input>
          </el-descriptions-item>
          ...
          <el-descriptions-item label="归档号">
            <el-input v-model="doc.recordNo" placeholder="请输入归档号" size="mini"></el-input>
          </el-descriptions-item>
...
export default {
  ...
  computed: {
    ...
    currentHandler() {
      if (!this.doc.opinions || this.doc.opinions.length === 0) return '无后续流转';
  ...
  methods: {
      ...
      this.$request.get('/flowObject/all')
          .then(res => {
            if (res.code === '200') {
              ...
              if (currentUserId && this.flowObjects.some(o => o.id === currentUserId)) {
    ...
    saveDoc() {
      this.$request.put('/outgoingDoc/update', this.doc)
          .then(res => {
            if (res.code === '200') {
    ...
    saveOpinionEdit(index) {
      if (index < 0 || index >= this.doc.opinions.length) return;
      ...
      // 调用API保存到数据库
      this.$request.put('/outgoingOpinion/update', originalOpinion)
          .then(res => {
            if (res.code === '200') {
    ...
    removeOpinion(index) {
      this.$confirm('确定要删除这条意见吗?', '提示', {
        ...
        const opinion = this.doc.opinions[index];
        if (opinion.id) {
          this.$request.delete(`/outgoingOpinion/delete/${opinion.id}`)
              .then(res => {
                if (res.code === '200') {
    ...
    saveOpinion() {
      ...
      if (!this.newOpinion.level) {
        this.$message.warning('请选择意见等级');
        return;
      }
      ...
      this.$request.post('/outgoingOpinion/add', this.newOpinion)
          .then(res => {
            if (res.code === '200') {
              ...
              this.doc.opinions.push(addedOpinion);
